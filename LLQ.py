...
[0.0017709732055664062, 0.0010221004486083984, 0.0010809898376464844, 0.0011439323425292969, 0.0009989738464355469,
 0.0008959770202636719, 0.0009720325469970703, 0.0008141994476318359, 0.0008640289306640625, 0.0008099079132080078]

def task(env, name, burst_time, ready_queue):
    yield env.timeout(burst_time)
    print(f'{name} finished at time {env.now}')

def fcfs_scheduler(env, tasks):
    ready_queue = simpy.Store(env)
    
    for task_name, burst_time in tasks:
        env.process(task(env, task_name, burst_time, ready_queue))
